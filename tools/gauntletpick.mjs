 ...
 *
 * На продуктовом поле — синяя сторона против оранжевой, свои мозги, кулдаун
 * из цены — они схлопываются в ЛЕСТНИЦУ: `turtle` бьёт всех, `controller` не
 * бьёт никого, между ними один порядок силы. Замерено `tools/gauntletfield.mjs`:
