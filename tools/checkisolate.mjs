 ...
 *
 * Шестая стена (D191 §1, играет ли мозг против спарринга — победа или 40%
 * урона, не простаивать, не стоять, кастовать не реже раза в 3 с) сюда не
 * попадает: она считается ТОЛЬКО когда пробе выдан набор (`admit(..., {kit})`,
 * тот же принцип, что у `never_uses`), а этот контроль, как и раньше, идёт
 * без кита — на голой фикстуре. Ей нечего добавить к тому, что здесь
 * проверяется: не ловят ли стены безопасности честный код.
 */
