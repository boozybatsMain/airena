...
const SEEDS = [
   ...
   * поровну разложена по двум ЗАМЫСЛАМ ТЕЛА — лёгкому дальнобойному и
   * тяжёлому ближнему. Видов и архетипов нет: это описания, которые модель
   * читает и по которым сама считает шесть чисел тела. Перекос в одну сторону
   * оставил бы ярус однобоким — все дерутся одинаково.
   *
