...
COPY tools ./tools
COPY bodies ./bodies
# Грамматика тела. БЕЗ НЕЁ СУЩЕСТВО РОЖДАЕТСЯ В ЧУЖОМ ТЕЛЕ, и молча: конвейер
# ловит отказ, ставит стоковое тело и продолжает (D117). Замерено на проде
# 05.09 — «мощный крокодил» вышел осьминогом, а в заметке рождения лежало
# `Could not resolve "/app/packages/forge/src/index.ts"`. Каталог весит 336 КБ;
# его отсутствие стоило генерации, за которую уже заплачено.
