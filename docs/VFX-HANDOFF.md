...
- **Viewer** (`src/viewer/main.js`): composite post graph (HDR bloom before ACES, chromatic aberration, heat distortion from a third MRT target, vignette, in-shader flash; `?post=0` or `?bloom=0` disables; WebGL2 verified to render it); trauma camera shake with roll applied after the framing solver; red hit flash driven by `hit` and targeted `impact` events (not hp deltas: burn and sudden death drain hp every frame), drawn as a solid red silhouette of the struck body rendered into its own texture with an override material and composited as the last step of the post graph (changing `material.color` does nothing on node materials, and a second render into the canvas blacked out the frame on WebGPU); light pool of four; `charge` records fired from the telegraph wind-up; sweep hooks `window.__airenaSweep` under `?vfx=1`.
