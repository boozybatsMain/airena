...
# Handoff 04.09: «эффект сработал — и его нет», выпуск четырёх стихий, бой моделями

Заказ основателя целиком записан в `docs/VFX.md` §8; здесь — что сделано, чем
измерено и что осталось.

## 1. Правило слоя вместо дисциплины десяти файлов

До этой сессии «след не должен переживать эффект» держалось тем, что каждый
модуль помнил про это сам. Не удержалось: после общей чистки в дереве всё ещё
лежали 20 с у штатного луча Nova, 16 и 10 с у времени и — то, на что никто не
смотрел — **21.5 с у КАЖДОЙ формы молнии**, и у каждого места был свой довод,
почему именно ему можно.

Теперь это три числа в `kit.js`, читаемые гейтом, README и модулями:
`DECAL_HOLD_MAX` 8 с (зажим на входе `decal`), `TAIL_MAX` 5 с, `FADE_MIN` 0.6 с.

Плюс три общих правки поля декалей: уход сглажен полиномом `3t²−2t³` с обоих
концов (был линейный, с изломом производной в начале и в конце), потолок
плотности по типу (`DECAL_PEAK`) и потолок свечения (`DECAL_GLOW`), которое
вдобавок гаснет по КУБУ огибающей — пол перестаёт быть источником света задолго
до того, как метка исчезнет.

И одна, найденная агентом молнии уже по ходу: **жар метки остывал по глухой
константе** (ожог 6 с, разряд 2.5, оплавление 1.5). Пока метки лежали по
двадцать секунд, это было безразлично; при выдержке 1.6 с ветка `arc` к
середине жизни отдавала восемь десятых тёмного — след успевал стать пылью,
будучи ещё полностью непрозрачным. Жар привязан к собственной жизни метки.

## 2. Гейт `tools/checkdecay.mjs`

Гоняет 90 форм (десять стихий × их формы плюс удар, статус, заряд) вхолостую на
НАСТОЯЩЕМ `Vfx` — без браузера и WebGPU — и меряет, когда гаснет последнее, что
форма попросила нарисовать: следы через наблюдателя `kit.watchDecals`, меши
через `spawnMesh`, частицы прямо из чередующегося буфера пула. 339 утверждений.

Три вещи в нём стоили отдельной работы и записаны на месте:

- **Гоняется живой `Vfx`, а не заглушка.** Первая версия подставляла объект
  руками и разошлась с предметом на первом прогоне: у живого `Vfx` поле `add` —
  это пул частиц, а не `scene.add`, и 46 форм упали с «vfx.add.emit is not a
  function». Заглушка, которая расходится с предметом, проверяет заглушку.
- **`three` → `three/webgpu` хуком разрешения.** Во вьювере это делает
  importmap страницы; в node тот же специфер даёт сборку без узловых
  материалов, и гейт молча пропускал 78 форм из 90.
- **Часы переводятся вперёд на вызове `update`.** Метку кладут и из ХОДА формы
  (лужа навеса — когда колба разбилась); гейт, читавший только `at`, считал её
  родившейся в нуле. Поймал это агент молнии: удар зоны с неба гаснул на 8.3 с
  при зоне 3.0 — хвост 5.3 при потолке 5.0, а гейт был зелёным.

Рядом `tools/vfxclean.mjs` — доля арены, занятая эффектом, против по-настоящему
пустого кадра (`nil/charge`: `--el=nil` пустоты НЕ даёт, стихии без модуля
падают на штатный силуэт, и только у `charge` он возвращает false).

## 3. Метка эффекта: «какой», а не «чей»

У УДАРА подписи атомов были всегда (`atomImpact`, четырнадцать штук). У СТАТУСА
не было: все десять модулей рисуют статус своей СТИХИИ, и оглушение,
обездвиживание, ослепление и немота выглядели одинаково — зритель видел «на нём
горит», но не видел «он не может ходить».

`Vfx.effectMark` рисует пять жестов поверх элементного статуса тем же приёмом,
которым уже живут удар, прыжок и стена: кольцо над головой (оглушение), скобы у
ног (обездвиживание), полоса штрихов поперёк глаз (ослепление), перечёркнутая
планка (немота), шевроны вверх/вниз (усиление/ослабление). Тонко, обычным
блендингом, без свечения, вход и выход тем же полиномом. Щит, лечение, очищение
и горение метки не получают, и это решение: полупрозрачная сфера рядом со щитом
читалась бы вторым щитом (урок судей), а лечение и очищение мгновенны.

## 4. Выпуск ЧЕТЫРЁХ стихий (шаг 11 плана §7.5) — и почему не пяти

Гравитация, кислота, радиация и лазер выпущены. Без этого «как бы я ни создавал
существо, оно всегда генерировалось с правильными скиллами» невыполнимо по
построению: пока флаг стоял, `grammar()` не показывал стихию модели вовсе, и
четыре из десяти модулей вьювера не мог получить ни один игрок.

**ВРЕМЯ ВЫПУЩЕНО НЕ БЫЛО, И ЭТО ИСПРАВЛЕНИЕ МОЕЙ ОШИБКИ.** Сначала я снял флаг
со всех пяти сразу — время попало «заодно», под общий довод про правильные
скиллы. Заказа на него не было, а был обратный, и основатель повторил его
прямым текстом: «я просил отключить время — чтобы времени вообще не было в
скиллах, чтобы никто не мог создать время». Флаг возвращён, четыре существа со
временем выведены из игры (`retired`, история цела): ХРОНОМЕТР, ХРОНОГРОМ и два
ЧАСОВЩИКА-ГРОМОВЕРЖЦА, 3 951 бой на четверых.

Потянуло за собой: счёт прочтений 495 → 654 (перебором, а не умножением — E1
делает стихии неортогональными; доля времени в этом счёте — 24 прочтения),
порог в `selfTest` и `checkspec` 5 → 9, три замороженные строки `SPEC.md`. Механизм `unreleased`/`releasedElements` НЕ
удалён — он пригодится следующей стихии; гейты `checkgrammar` и `checkforge`
переписаны так, чтобы проверять КАЛИТКУ (флаг ставится временно и снимается в
`finally`), а не конкретного жильца, который выпуском исчезает.

## 5. Правильные скиллы — замером, а не на слово

Промпт кузницы теперь называет каждой стихии её `read` («тяжесть и притяжение»,
«разъедание») и требует брать стихию БОЛЬШИНСТВОМ набора, если описание
однозначно её называет.

Замер: описание «Стужа: выстуживает воздух, сковывает наледью, делает хрупким;
дерётся холодом, а не силой» ДО правила дало 1 умение из 3 на морозе — конус и
рывок вышли серой кинетикой. ПОСЛЕ — 3 из 3. По волне из четырнадцати описаний
стихия не промахнулась ни разу; у лазера и радиации 2 из 3 — это потолок, а не
промах: E1 не даёт им доставки `self`, и третий слот обязан быть чужим.

`tools/seedelements.mjs` — этот прогон целиком: описание, в котором нет ни
одного слова из грамматики, → разбор моделью → набор → мозг → тело → игровое
существо. `tools/duel.mjs` — настоящий бой названной пары тем же `playMatch`,
что и у сервера, со ссылкой на повтор и с числом применений каждого умения.

## 6. Волна судей, круг 1 (10 стихий, 0–100 против заказа)

| стихия | всего | захламление | красота | отличимость |
|---|---|---|---|---|
| время | **84** | 86 | 82 | 84 |
| дуга | **80** | 82 | 77 | 84 |
| мороз | **79** | 80 | 76 | 82 |
| гравитация | **77** | 88 | 73 | 70 |
| жар | **74** | 80 | 68 | 72 |
| кинетика | **71** | 83 | 62 | 66 |
| пустота | **69** | 66 | 64 | 78 |
| кислота | **66** | 76 | 63 | 58 |
| радиация | **66** | 84 | 56 | 58 |
| лазер | **62** | 68 | 56 | 63 |
| **среднее** | **73** | **79** | **68** | **72** |

Читать эту таблицу надо по столбцам, а не по строкам. **Захламление —
79 в среднем и ни одной оценки ниже 66:** заказ, с которого всё началось,
выполнен, и выполнен ровно там, где был провален (у лазера 68 — потому что
статус ещё горел на четвёртой секунде, а не потому, что след лежит долго).
Проваливаются другие два столбца — **красота 68 и отличимость 72**, — и это
честный результат: слой перестали захламлять раньше, чем успели довести.

Что судьи назвали чаще всего, дословно по смыслу:

- **род вещества путается.** Кислота и радиация делят один алфавит меток
  (рваные кляксы 4–10 px, различает только цвет); кинетический конус читается
  «разлитой синей краской»; пустота — «цветной лужей».
- **эффект светит на весь кадр.** У жара дальняя стена в двадцати метрах от
  очага поднимается на +74 уровня RGB против чистой плиты (у всех прочих
  стихий +0.0…+1.1). У пустоты снаряд размером в десять пикселей давал
  хроматическую кайму у каждого ящика и обоих бойцов.
- **форма не оставляет следа вовсе.** Болт кислоты — 224 изменённых пикселя на
  трансляции; болт молнии — ноль, кадр байт-в-байт равен пустой арене; конус
  радиации — 760 против 24 775 у кислоты в тот же момент.
- **плавность ухода НЕ ДОКАЗАНА.** Между 1.5 и 4.0 с в прогонах не было ни
  одного кадра, и судьи отказались засчитывать плавность на слово. Момент 2.6 с
  добавлен во все дальнейшие прогоны — это правка методики, а не картинки.

## 7. Круг 2: чинилось ровно то, что назвал судья

Семи стихиям, которым первый круг поставил ниже 78, был отдан ЕГО ЖЕ список
`must_fix` — дословно, пунктом за пунктом, — и одно правило: по каждому пункту
либо починка с замером, либо отказ с доводом. Отказ считался законным ходом:
у судьи бывает неправ замер, а у пункта — цена выше пользы.

| стихия | круг 1 | круг 2 | захламление | красота | отличимость |
|---|---|---|---|---|---|
| гравитация | 77 | **80** | 90 | 74 | 76 |
| кинетика | 71 | **79** | 86 | 77 | 81 |
| пустота | 69 | **79** | 84 | 76 | 77 |
| лазер | 62 | **77** | 84 | 72 | 78 |
| жар | 74 | **74** | 82 | 72 | 68 |
| кислота | 66 | **74** | 79 | 68 | 70 |
| радиация | 66 | **73** | 88 | 57 | 64 |
| **среднее семи** | **69** | **77** | | | |

Вместе с тремя, прошедшими сразу (время 84, дуга 80, мороз 79), десять стихий
дают **78**.

Семь агентов закрыли 43 пункта и отклонили 34 — почти половину, и почти все
отказы одного рода: «это не мой файл». Из них вышел список правок ведущего,
и каждая — настоящий дефект, найденный чужими глазами:

- **сетка арены писала глубину.** Прозрачные линии на высоте 0.012 м резали
  всё, что лежит ниже: шрам пустоты протекал полом ровно по клеткам. Нашёл
  агент пустоты по кропу 125×55.
- **хроматическая аберрация красила весь кадр.** Снаряд в десять пикселей давал
  жёлто-красную кайму у каждого ящика, у стены арены и у обоих бойцов.
  Коэффициент ополовинен: у края смещение было пятнадцать пикселей на ширине
  1600.
- **радиация и кислота делили один алфавит меток** — обе рисовались порогом по
  фрактальному шуму, и различал их только цвет. У радиации теперь СЫПЬ ПО
  РЕШЁТКЕ: `q` квантуется в клетки, у каждой свой сдвинутый центр и свой
  радиус, между крупинками чистый пол по построению.
- **у языка пламени был плоский срезанный низ:** множитель вертикали у нижней
  кромки квада падал до 0.3, маска доезжала до края и обрывалась прямым углом.
- **`kit.shockwave` всегда рисовал светящееся кольцо** — стихии без свечения
  платили чужим накалом, и кинетике пришлось завести свою волну в сорок строк.
  Появился `hot: false`.
- **запись зоны не несла своих атомов**, поэтому ни `vfx.zonePull`, ни модуль
  не могли показать зону, которая ТЯНЕТ. Поле добавлено в `deliver.js` и в
  стойку `vfxfixture`; штрихи летят к КАСТЕРУ, а не к центру круга, потому что
  туда же тащит `applyEffect`.
- **строка гейта про хвост статуса ничего не проверяла.** Носитель статуса
  продлевается симом, а гейт зовёт его обновление один раз — разность выходила
  отрицательной и пропускала всегда. Строка снята с честной пометкой, а не
  подкручена.

## 8. Волна приёмки: восемь судей, восемь углов, одна оценка за всё ТЗ

Круг по стихиям кончился на 78 в среднем, и это была оценка ПО СТИХИЯМ. Заказ
же написан не про стихии, а про слой: «не захламляет», «красиво», «функция
совпадает с визуалом», «существо всегда генерируется с правильными скиллами».
Поэтому вторая волна судила иначе: восемь судей, у каждого свой угол — позднее
затухание, красота зон, красота дальних форм, различимость стихий, подписи
атомов, настоящий бой, контактные формы, техническая часть генерации, — но
оценку каждый ставит ЗА ВСЁ ТЗ со своего угла.

**71 в среднем, худшая 58.** И это правильный результат, потому что он развёл
две вещи, которые оценка по стихиям складывала в одну.

**Что подтверждено числом.** Доля арены, занятая эффектом на t = 4.00 с (замер
против по-настоящему пустого кадра): **среднее по 60 формам 0.12 %, у 52 из 60
ровно 0.00 %**. В записи НАСТОЯЩЕГО боя на 20-й, 30-й и 40-й секунде пол чист —
накопления нет вообще. Уход именно скатом: ember/zone 14.1 → 12.0 → 11.3 → 6.6
→ 3.4, void/zone 3.66 → 3.67 → 1.65 → 0.36 → 0.07. Головная жалоба основателя
снята, и снята измеримо.

**Что судьи нашли, чего не видел никто.** Список ниже — не придирки, а дефекты
с файлом, строкой и числом; каждый закрыт в этом же круге:

- **`MARKS` — глобальная карта без уборки.** Жест эффекта записывался в неё
  навсегда; оставшаяся от кинетики запись возвращала ранний `true`, и у мороза
  с гравитацией жест не рисовался ВООБЩЕ. Запись теперь снимается вместе с
  мешем.
- **Жест жил `duration`, а статус — дольше.** Сим продлевает статус тиками,
  `until` растёт, а меш умирал по своей длительности и больше не рождался:
  замер судьи — двадцать пять применений за двенадцать секунд дали одну метку
  на четыре секунды. Носитель теперь гасит `until`, а не срок.
- **`case 'stun'`: `const y = (at ? 0 : 0) + 2.35`** — капсула жертвы
  вычислялась и выбрасывалась, высоты жестов были числами. У низкого
  четвероногого кольцо висело в метре над спиной.
- **Немота говорила разными словами.** Подпись удара рисовала одну планку,
  метка статуса — две крест-накрест. Теперь обе перечёркнуты.
- **Семь подписей из четырнадцати брали случай из `Math.random`** — прямой
  запрет §9 и разъезд повтора с боем.
- **Штатная зона гасла 0.54 с линейно** — ниже собственного порога слоя
  (`FADE_MIN` 0.6) и не тем полиномом, которым уходит всё остальное. Это самая
  крупная поздняя фигура набора: штатная зона лазера, 12 % арены на 2.6 с.
- **Носители статуса уходили ВЫКЛЮЧАТЕЛЕМ** — `visible = false` у пустоты,
  льда и молнии. Рецепт был написан в `time.js` ещё в прошлом круге; его не
  перенесли. Теперь перенесён во все три.
- **Вспышка урона заливала оба тела плоским красным** (5.10 % и 4.36 % арены,
  больше любого эффекта в записи) и стирала элементный статус ровно в момент
  попадания. Полка срезана 0.12 → 0.05 с, потолок 0.95 → 0.72.
- **Гейт не видел половины того, что рисуется.** Он шёл по `ELEMENTS[x].forms`
  — списку ЗАКОННЫХ доставок, — а на экране есть и штатные силуэты: laser/zone,
  acid/beam, time/beam и ещё четыре пары в него не входили. Теперь гоняются все
  девять доставок у каждой стихии.
- **И гейт не мерил уход статуса вовсе.** Теперь мерит — и не мгновенную
  непрозрачность, а ОГИБАЮЩУЮ (скользящий максимум): у пульсирующего статуса
  молнии вход обязан быть мгновенным, и запрещать это значило бы запрещать
  ритм. Требование — от половины рабочего уровня до нуля не быстрее чем за
  0.3·`FADE_MIN`; арифметика полинома в комментарии на месте.

## 9. Круг 3 и находка, которую нашла бисекция, а не глаз

Волна приёмки назвала «сплошную серую плоскость поверх кадра в миг удара», и
это выглядело как вкусовщина, пока агент мороза не разобрал её числом.

Замер: тёмный задник кадра поднимается на **+22…+34 уровня RGB** в момент
удара — при **+0.0…+1.8** у девяти стихий, которые в тот же миг не били. Белый
пол при этом поднимается на +0.7…+1.0. То есть это ПЛЁНКА ПОВЕРХ КАДРА, а не
свет и не частицы.

Прогон с `?bloom=0` разделил вклад: четверть даёт экранная заливка
(`vfx.screen.flash`), три четверти — **вклад удара в свечение**. Причина
арифметическая и была в общем наборе: `kit.impactKit` зажигал источник
`22·strength` (до 35) радиусом `radius·5` — десять метров света ради воронки в
два, — и мип-пирамида блума размазывала пик по всему кадру.

Мороз и огонь уже завели себе локальные обходы (`frostHit`, `fireHit`);
остальные восемь стихий звали набор как есть, и у любой, чей удар совпадал с
судейским мигом, была та же плёнка. Починено в одном месте: пик 22 → 13,
радиус ×5 → ×2.5, кайма 0.5 → 0.18. Свет остался тем, чем заведён, —
подсветкой тел и пола вокруг точки, — и перестал быть засветкой сцены.

Остальное круга 3, по стихиям:

- **мороз** — самый долгий хвост слоя закрыт: иней тает с мига, когда клин
  ЛОПНУЛ, а не когда осел последний кристалл (конус 4.1 → 3.2 с, на t = 4.00
  ровно 0.000 % арены на всех трёх ракурсах). У ледяного дождя появился
  источник: устье опущено с 9 до 5.6 м — внутрь силуэта арены, — и в нём
  встала туча из пара. Пикселей эффекта выше кромки стены: 19 332 → 0.
- **пустота, радиация, кинетика** — свои списки от волны приёмки; отчёты в
  журнале прогона.

Отдельно про методику: контактный лист теперь подписывает клетку «вне
грамматики», если пара «стихия × доставка» запрещена правилом E1. Прогон
снимает декартово произведение, и пустой кадр такой пары читался как
«сломанный эффект» — судья первого круга честно так и написал.
