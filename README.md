...
npm test                                     # 70 invariants, then every gate in order
...
tools/         anglicize · arena · bake · balance · bench · bodyinstall · bracket · brainforge ·
               ...
               checkfaults · checkdescribe · rethink ·
...
| `node tools/test.mjs` | 70 invariants — determinism, collision, sandbox, grammar mechanics, wire |
| `node tools/checkprompt.mjs` | the prompt and the config still agree, in both directions and in both forms of the document — the reference fixture's and the one a player's creature reads, whose skill cards come from its compiled kit. A card number carries a `kit.<side>.<slot>.<field>` label, is re-derived here from the compiled ability, and everything outside the emitted values is swept for stray numerals. It also runs the tactics judge |
| `node tools/checkbehaviour.mjs` | the world does what the prompt says: every reach and cone by binary search, and every timing by counting ticks — for the reference fixture and for a compiled kit, where it also measures the served wind-up and recovery, the cooldown, the mortar's splash, the field's tick schedule and its whole-disc totals, the blink's invulnerability, how far a knock and a pull really move a body, how long an immunity window lasts, the heal's floor, cap and share, and the leap's phases |
...
| `node tools/atombalance.mjs` | the weight of every grammar piece from measured win rates: random legal kits, a league on the multi-pilot panel (`brains/pilots/`), a ridge regression of score on the pieces, and a proposed cost table. `--n`, `--games`, `--pilots`, `--cooldown`, `--seed`, `--out` |
| `node tools/bakeoff.mjs forge --models=… --creatures=reports/combat/bakeoff/creatures.json` · `node tools/bakeoff.mjs league` | the model bake-off: the same three creatures given minds by many models (OpenRouter ids and `sub:` bundles), admitted like a player's, then a league against each other and against the pilot panel with the spectator metrics per mind and per model |
...
| `node tools/checkfixtures.mjs` | a third of the sides on the public ladder were a body that walked for the whole fight and cast nothing, and a fight like that is indistinguishable from an honest loss — same outcome, same length, same cause of death. Two things produced it: a creature with no grammar kit was handed one of the two reference sets by the COLOUR it was placed on (the parity of the match seed), while its mind was written against exactly one of them and calls those verbs by name; and the database copies of the handwritten reference still walked their kit behind `if (k.trigger !== 'active')`, a field the grammar stopped sending. This runs every live library and every kitless creature against the kit-stub, on BOTH colours, over two seeds, and fails if any of them casts fewer than three times — both colours because half the ladder ran on the "wrong" one and a one-colour check would have missed the entire defect. `--falsify` hands the fixture out by colour again and reproduces it exactly: every kitless creature drops to zero on one side. Two minds that stay mute for a DIFFERENT reason — they gate their attack on the cooldown of an enemy `smash`/`charge` that a grammar kit does not have — are listed by name with their measurement, and the list fails the gate if one of them ever starts casting, so the exception cannot outlive its cause |
| `node tools/seedfixtures.mjs` | *(seed tool, not a gate)* the write half of the same repair, and the only thing in the tree that sets `creature.reference_tag`. It infers the tag from the mind itself — from which verbs the mind hands to `api.use/ready/cooldown` — and refuses to guess when a mind calls both halves or neither. It also re-seeds `brain_source` for the handwritten reference rows from `brains/kit-stub/`, keyed on PROVENANCE rather than on the name: no owner and the library flag set, so a player's creature can never be rewritten by it. Takes its own database snapshot first, deletes nothing, and `--dry` prints the whole plan with before/after hashes |
...
| `node tools/vfxseq.mjs` | capture of a SEQUENCE of `world.fx` records with delays, the way the sim writes them: a bolt and the impact of the same (who, skill) pair, `self` with `status: shield`, a zone re-applying its atoms |
