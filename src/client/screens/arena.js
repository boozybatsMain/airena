...
export async function pickStarter(box, ctx) {
  ...
  for (const c of list) {
    ...
    h('div.hd', c.name),
    /*
     * Строка под именем описывала ВИД: «тяжёлое, ближний бой». Видов нет, и
     * существо, названное игроком «стеклянная медуза», читало про себя чужую
     * характеристику. Теперь строка говорит про ТЕЛО, которое существо себе
     * выбрало, — то есть про его собственные числа.
     */
    h('div.why', c.bodyLine || 'тело обычное'),
