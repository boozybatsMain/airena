...
export async function enter(root, args, ctx) {
   ...
   * Спросить код.
   *
   * Гость здесь не отсекается заранее по `session.guest`: право поднимать
   * воркер знает только сервер (список ведётся руками), и второй, клиентский,
   * ответ на тот же вопрос разошёлся бы с первым при первом же изменении
   * списка. Один источник правды — ответ ручки.
   */
   ...
   * так отвечают почти всем, кто сюда зайдёт. Красный остаётся тому единственному
   * случаю, где что-то действительно сломалось.
   */
  function paintDeny(e) {
    clear(box);
    const guest = e?.status === 401 || e?.code === 'guest';
    ...
    if (guest) {
      box.appendChild(h('div.t-body',
        ...
        + 'отдавать сборку. Войди — и код появится здесь же.'));
      box.appendChild(h('div.row', { style: { marginTop: '14px' } },
        h('button.btn.big', { onclick: () => ctx.go('/save') }, 'ВОЙТИ')));
      return;
    }
