...
export async function enter(root, args, ctx) {
  ...
  /*
   * ГОСТЬ СОЗДАЁТ НАРАВНЕ. СТЕНЫ АККАУНТА БОЛЬШЕ НЕТ (05.09).
   *
   * Здесь жила развилка `guest`: экран показывался целиком, но «СОЗДАТЬ»
   * уводило на модалку «СОХРАНИТЬ СУЩЕСТВО» и генерация начиналась только
   * после привязки аккаунта платформы. Решение основателя — снять целиком:
   * промпт, кнопка, генерация.
   *
   * Что это отменило из написанного раньше: D1 («генерация гостю недоступна,
   * потому что стоит живые деньги») и F7 («одно бесплатное существо на
   * аккаунт пожизненно»). Деньги теперь сторожат только лимиты E3, и
   * настоящий потолок среди них один — дневной бюджет игры: гостевую запись
   * заводит любая очистка хранилища браузера, а бюджет общий и не обходится.
   */
  ...
  const errBox = h('div');
  const goBtn = h('button.btn.primary', { onclick: submit }, 'СОЗДАТЬ СВОЁ');
  ...
  async function submit() {
    const prompt = field.value.trim();
    /*
     * СТЕНЫ АККАУНТА ЗДЕСЬ БОЛЬШЕ НЕТ (05.09, решение основателя).
     *
     * Отсюда игрок уходил на `/save` — модалку «СОХРАНИТЬ СУЩЕСТВО», — и
     * генерация начиналась только после привязки аккаунта платформы. Экран
     * снят целиком вместе с маршрутом: написал промпт, нажал «создать», пошла
     * генерация. Гость создаёт наравне (`limits.check` на сервере).
     */
    goBtn.disabled = true;
    clear(errBox);
