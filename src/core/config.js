 ...
 * length, and levelling the two top speeds (see FIGHTERS) lengthened the mean
...
/**
 * Характеристики существа этого архетипа при этом размере.
 *
 * Возвращает НОВЫЙ объект: `FIGHTERS[id]` — общая на весь процесс запись, и
 * править её под конкретного бойца значило бы менять её для всех сразу.
 * Ровно на этом однажды сломался замер: `tools/matchworker.mjs` мутировал
 * `FIGHTERS.gorilla`, и это работало только потому, что жило в отдельном
 * процессе.
 */
export function statsFor(id, size = 1) {
  const base = FIGHTERS[id];
  const k = Math.max(SIZE_MIN, Math.min(SIZE_MAX, Number.isFinite(size) ? size : 1));
  /*
   * Единица идёт ТЕМ ЖЕ путём, что и остальные размеры.
   *
   * Здесь стоял ранний возврат `if (k === 1) return base` — «ничего не
   * меняется, зачем считать». А менялось: у единицы здоровье оставалось
   * ровно базовым (155), у всех прочих проходило через `Math.round`. На лиге
   * размеров единица оказалась худшим размером из пяти при обоих соседях
   * около половины — не потому, что она хуже, а потому, что считалась иначе.
   *
   * Правило шире случая: ветка «частный случай, тут ничего не меняется» —
   * это второй путь исполнения, и он расходится с первым.
   */
  return {
    ...base,
    size: k,
    hp: Math.round(base.hp * k ** SIZE_HP_POW),
    /* Множитель урона: применяется в `damage()`, а не в атомах, — иначе его
       пришлось бы дублировать в каждом из четырнадцати. */
    dmgScale: k ** SIZE_DMG_POW,
    radius: base.radius * k,
    maxSpeed: base.maxSpeed * k ** -SIZE_SPEED_POW,
    /* Масса растёт с объёмом: отброс мелкого дальше, и это тоже часть цены. */
    mass: base.mass * k ** 3,
  };
}
...
tune(FIGHTERS, 'fighters');
tune(SKILLS, 'skills');

/** Which skills a fighter may name, jump included. */
export function skillsOf(fighterId) {
  return [...FIGHTERS[fighterId].skills, 'jump'];
}
