...
function kitView(f) {
  ...
  for (const [name, d] of Object.entries(f.kit)) {
    out[name] = {
      ...
      /*
       * Круг поражения навеса в точке приземления. Без него F10 отдаёт
       * умение, которым НЕЛЬЗЯ ПОЛЬЗОВАТЬСЯ ОСОЗНАННО: с 04.09 навес не
       * задевает никого в полёте и бьёт только по кругу там, где упал, —
       * то есть мозг, знающий одну лишь `range` (предел), видит верхнюю
       * границу приказа и не видит цены ошибки в этом приказе. Второе число
       * той же механики, и оба обязаны быть в перцепции, а не только в
       * промпте: набор меняется без регенерации мозга.
       */
      ...(d.splash !== undefined ? { splash: round3(d.splash) } : {}),
...
function startSkill(world, id, name, a, b) {
  ...
  /*
   * ВТОРОЕ ЗНАЧЕНИЕ ПЕРВОГО АРГУМЕНТА: дальность навеса.
   *
   * `api.use(name, a, b)` принимал два числа с самого начала, а пользовалось
   * ими только мигание — у остальных восьми доставок аргументы молча
   * пропадали. Заказ основателя 04.09: «существо может контролировать
   * рейндж [лобной атаки], и должно это делать, чтобы попадать». Для навеса
   * `a` — ЗАПРОШЕННАЯ ДАЛЬНОСТЬ в метрах.
   *
   * Здесь она только ЗАПОМИНАЕТСЯ, сырая. Зажимает её `deliver.js` в момент
   * приземления каста, и это не лень, а то же правило, по которому в
   * `resolveStrike` берётся курс: замах длится полсекунды, за неё истекает
   * усиление канала «дальность» и меняется предел. Зажать на старте значило
   * бы обещать мозгу предел, которого к удару уже нет.
   *
   * Число НЕ проверяется на осмысленность и не отвергается: границы —
   * механика доставки, а не право на приказ, и `lobLanding` их применяет
   * молча. Отказ здесь означал бы, что умение с 0.5 с замаха ушло в кулдаун
   * из-за арифметики мозга.
   *
   * ОДНО ЧИСЛО — ДАЛЬНОСТЬ, ДВА — НАПРАВЛЕНИЕ, И ЭТО НЕ ИЗЯЩЕСТВО, А ЗАМЕР.
   *
   * Пара `(a, b)` уже занята: она значит НАПРАВЛЕНИЕ, так её ввело мигание и
   * так её пишут мозги. В базе 186 вызовов `api.use` у 77 существ: 127 без
   * аргументов, 59 — с парой вида `api.use(best.name, en.x - me.x,
   * en.z - me.z)`, и РОВНО НОЛЬ с одним числом. Прочитать первое число пары
   * как метры значило бы, что у ГРОЗЫ и ПЕПЛА (обе шлют пару) навес улетает
   * на разность координат: замерено — 24 навеса из 26 легли на нижнюю
   * границу 3.3 м при враге в 13.5 м, то есть себе под ноги.
   *
   * Поэтому дальностью считается только ОДИНОКОЕ число. Пара по-прежнему
   * значит направление, дальность в ней не названа — и работает умолчание,
   * то есть навес ложится на врага. Ни одного существующего вызова это не
