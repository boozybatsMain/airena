...
class DecalField {
  constructor(scene, type, palette) {
    ...
    if (type === 'soot') {
      ...
      const glassC = mix(mix(tint.mul(0.9), vec3(1.05, 1.06, 1.1), grain.mul(0.35).add(0.5)), vec3(1.2, 1.22, 1.3), rays.mul(0.6));
      ...
      const blot = smoothstep(float(0.62), float(0.78), mx_fractal_noise_float(p3.mul(7.0).add(1.7), 3, 2.0, 0.5, 1).mul(0.5).add(0.5));
      ...
      alpha = core.mul(blot.mul(0.86).add(0.06)).mul(0.92);
