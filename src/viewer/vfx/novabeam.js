...
export function beam(vfx, e, P, ctx) {
  ...
  const S = kit.tune(e, {
    ...
    decalHold: 20,           // стойкость ожога, с: ожог — ОСТАТОК, живёт после луча
  ...
  const onHit = () => {
    ...
    if (hit) {
      ...
      /* Ожог — ОСТАТОК: он обязан пережить луч, поэтому `hold` свой, а не от
         `duration`. Радиус — 0.47 кольца удара: ожог всегда меньше вспышки,
         которая его оставила, и следует за ней, когда её крутят. */
      kit.decal(vfx, { type: 'laser', x: B[0], z: B[2], radius: S.impactRadius * 0.47, hold: S.decalHold, tint: mid, seed: (seed % 9) + 1 });
    } else {
