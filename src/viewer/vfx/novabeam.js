 ...
 * стоит тающий колокол; точки на полу ~2.9 с. Все эти числа — значения по
 * умолчанию ручек `kit.tune` в начале `beam`: держание задаёт `duration`,
 * хвосты едут за ним.
 *
 ...
 * поперечник трубы, лент и колец — от одной ручки `width` (по умолчанию
 * радиус луча сима × 1.2), длительности — от `duration`.
...
/**
 * Тайминги, секунды от каста (см. шапку), но не таблица чисел, а СМЕЩЕНИЯ от
 * ручек вызова: замах `windup`, прорастание `grow`, держание `duration`, а
 * дальше — замеренные на эталоне хвосты (луч гаснет за 0.3 с после полной
 * фазы, шар за 0.35, купол за 0.6: на 1.6 с над ковром ещё стоит тающий
 * колокол). Поэтому одна ручка `duration` двигает весь залп разом, а не
 * рвёт его на части.
 *
 * `windup` 0.2, а не 0.15: снимок «0.15» ложится на 0.150–0.156 с, и при
 * выходе ровно на 0.15 кадр ловил луч на треть пути (замер 2.1) — эталон на
 * 150 и 450 мс показывает только шар.
 *
 * Полы: `grow` не ноль, потому что `reachAt` делит на него (мгновенный луч —
 * это 0.01 с, а не NaN), и полная фаза обязана начаться ПОСЛЕ прорастания,
 * иначе `envK`/`envD` считают спад по отрицательному отрезку и выворачиваются.
 */
function timeline(S) {
  const out1 = S.windup + Math.max(0.01, S.grow);
  const full = Math.max(out1 + 0.1, S.duration);
  return { out0: S.windup, out1, full, off: full + 0.3, orbEnd: full + 0.35, dome: full + 0.6 };
}
...
function segMat(layer, P) {
  return pooled(`nova:seg:${layer}:${hex(P)}`, () => makeSegMat(layer, P), MAT_RING);
}
 ...
 */
function glassMat(P) {
  return pooled(`nova:glass:${hex(P)}`, () => {
    const { deep, mid } = tones(P);
    const m = new THREE.MeshBasicNodeMaterial({ transparent: true, depthWrite: false, side: THREE.FrontSide, blending: THREE.NormalBlending });
    const fade = withFade(m);
...
export function beam(vfx, e, P, ctx) {
  ...
  const sx = -uz, sz = ux;
  const U = [ux, 0, uz], SV = [sx, 0, sz], Y = [0, 1, 0];
  const REF = kit.REF_AREA.beam;

  /*
   * РУЧКИ ФОРМЫ. По умолчанию — ровно сегодняшние числа, поэтому пока запись
   * поля не несёт, кадр прежний. Ширина ведёт ВСЁ поперечное разом: трубу,
   * радиус винта лент, бегущие кольца и облако штрихов внутри. Порознь их
   * крутить нельзя: ленты с радиусом 0.7 при трубе 0.3 висят снаружи тумана,
   * а кольца перестают быть кольцами ВОКРУГ луча (замер 2.1: кольца обязаны
   * идти чуть снаружи трубы, иначе они режут её насквозь).
   */
  const S = kit.tune(e, {
    duration: 1.2,           // держание луча на полной силе, с
    windup: 0.2,             // замах: у руки только шар, луча ещё нет, с
    grow: 0.1,               // прорастание фронта от руки до цели, с
    width: fp.radius * 1.2,  // радиус трубы, м (радиус луча сима × 1.2)
    orbRadius: 0.6,          // шар каста у руки, м
    domeRadius: 1.25,        // колокол у цели, м
    impactRadius: 3.4,       // кольцо удара по полу, м
    impactLife: 0.5,         // жизнь кольца удара, с
    /*
     * ОЖОГ — ОСТАТОК ЛУЧА, И ОСТАТОК КОРОТКИЙ. Здесь стояло 20 с, и это тот
     * самый случай, на который основатель пожаловался 04.09: «эффект
     * сработал — пусть исчезает сразу после эффекта, не моментально, а
     * постепенно». Луч держится `duration` (1.2 с) плюс прорастание; ожог
     * пережил бы его в шестнадцать раз. Выдержка = длительность луча
     * (`null`), после чего пятно растворяется за `decalFade`: пол горяч,
     * ПОКА по нему бьёт луч, и остывает сразу, как только тот погас.
     */
    decalHold: null,         // выдержка ожога, с (null — ровно жизнь луча)
    decalFade: 1.6,          // растворение ожога, с
    dust: 2.4,               // рассев рождения ковра искр на полу, с
    dustRadius: 2.2,         // круг ковра у цели, м
  });
  const T = timeline(S);
  /*
   * Множители поперечных размеров: сегодняшняя труба 0.48 м, колокол 1.25 м,
   * шар 0.6 м. Всё, что сидит ВНУТРИ них (ядро, ореол, вынос ядра к точке
   * входа), едет вместе с ними — иначе на вдвое большем куполе ядро остаётся
   * прежней точкой и колокол читается пустым.
   */
  const wK = S.width / 0.48, dK = S.domeRadius / 1.25, oK = S.orbRadius / 0.6;
  /* Радиус ленты на выходе из шара — тоже доля ширины. */
  const RIB_IN = 0.15 * wK;
   ...
   * Радиусы — при штатной ширине; ручка `width` тянет их за собой (`wK`),
   * чтобы винт не вылезал из трубы. Ширины лент не трогает: они замерены в
   * пикселях (жила 4–6 px), а не в долях луча.
  ...
  for (let i = 0; i < nRib; i++) {
    ...
    ribs.push({
      ...
      r: wK * (kind === 'core' ? 0.1 + rng() * 0.12 : 0.26 + rng() * 0.52), pitch,
  ...
  /* Кольца идут в 5/3 ширины: чуть снаружи трубы, но внутри разлёта лент. */
  const RING_R = S.width * (5 / 3), RING_V = 5.5, RING_N = 40, FOOT_N = 48;
  const rings = [];
  const tmpA = [0, 0, 0];
  for (let q = 0; q < nRing; q++) {
    ...
    rot(SV, tmpA, ta, e1); rot(Y, tmpA, ta, e2);
  ...
  const helix = (rb, t, time, out, o) => {
    ...
    /* Лента выходит из шара: радиус `RIB_IN` → полный за первые 0.6 м. */
    const emerge = clamp01((t * len) / 0.6);
    const r = (RIB_IN + (rb.r - RIB_IN) * emerge) * (1 + rb.breathe * Math.sin(t * len * 1.7 + rb.ph2 + time * 3.0));
    const along = t * len;
  ...
  const writeField = (time, reach, kRib, kFoot) => {
    ...
    /* Контактное кольцо купола на полу: колокол стоит на полу весь удар
       (кольцо удара из набора живёт `impactLife`, а купол — до `T.dome`).
       Это ПРОЕКЦИЯ живого колокола, а не остаток: радиус — сжатый по
       `squash` радиус купола, и кольцо гаснет вместе с ним по `kFoot`.
       Тот же штрих: белая жила, тёмная рубашка; с хлыстами (u = 1). */
    if (hit && kFoot > 0.001) {
      const rr = S.domeRadius * 0.92 * (1 + 0.03 * Math.sin(time * 5.0));
  ...
  glass(vfx, P, { x: A[0], y: A[1], z: A[2], at: 0, r0: 0.15 * oK, r1: S.orbRadius, grow: 0.15, hold: T.full + 0.05, life: T.orbEnd, seed: (seed % 5) + 2, hot: 1.0, dense: 0.85, rim: 0.5, core: 0.22 * oK, halo: 0.5 * oK, detail: 3, edge: 0.03, edgeK: 0.2 });
   ...
   */
  const nSpike = kit.countFor(44, Math.PI * S.domeRadius * S.domeRadius, kit.REF_AREA.impact, 90);
  const onHit = () => {
    ...
    if (hit) {
      ...
      floorRing(vfx, [P[0], P[1], deep], { x: B[0], z: B[2], r0: 0.4, r1: S.impactRadius, life: S.impactLife, at: 0 });
      /* Ожог живёт РОВНО ПОКА БЬЁТ ЛУЧ (`T.full` — конец полной силы, минус
         уже прошедшее время удара), потом растворяется за `decalFade`. Радиус
         — 0.47 кольца удара: ожог всегда меньше вспышки, которая его
         оставила, и следует за ней, когда её крутят. */
      kit.decal(vfx, {
        type: 'laser', x: B[0], z: B[2], radius: S.impactRadius * 0.47, tint: mid, seed: (seed % 9) + 1,
        hold: S.decalHold == null ? Math.max(0.6, T.full - T.out1) : S.decalHold, fade: S.decalFade,
      });
    } else {
  ...
  if (hit) {
    /* Купол живёт дольше луча (до `T.dome`, по умолчанию 1.8 с): на 1.6 с
       над ковром ещё тает колокол, как у эталона на 1600 мс. */
    glass(vfx, P, { x: B[0], y: 1.1, z: B[2], at: T.out1, r0: 0.3 * dK, r1: S.domeRadius, grow: 0.14, hold: T.full, life: T.dome, squash: 0.92, seed: (seed % 7) + 1, hot: 1.0, dense: 0.5, rim: 0.85, core: 0.46 * dK, halo: 1.05 * dK, coreAt: [-ux * 0.6 * dK, 0.5 * dK, -uz * 0.6 * dK], detail: 3, edge: 0.02, edgeK: 0.12 });
  }
  ...
  if (hit) {
    const nTgt = kit.countFor(900, Math.PI * S.dustRadius * S.dustRadius, kit.REF_AREA.impact, 420);
    const tgtDot = (glow) => (i, s) => {
      const a = rng() * TAU, d = Math.sqrt(rng()) * S.dustRadius;
