...
export function jump(vfx, e, P, ctx) {
  ...
  /* ВЕЕР РАСТЁТ С ВЫСОТОЙ ПРЫЖКА. `height` в записи — это `jumpHeight` бойца
     ...
     МНОЖИТЕЛЬ ЗАЖАТ СНИЗУ, и это не перестраховка. Стенд кладёт в запись
     2.2 м, а СИМ кладёт `jumpHeight` бойца — у гориллы это 1.3 (`config.js`),
     при вилке стата 0.8–3.2. Без зажима веер выходил бы 0.71–1.30 м у
     гориллы и 0.44–0.80 м на дне вилки — ровно та полоса, про которую двумя
     строками ниже стоит замер: «четыре нити по 0.5–1.1 м с 26 м были
     невидимы, кадр 0.12 с пустой». Нижняя граница 0.88 держит веер не короче
     1.06 м, то есть над измеренной полосой невидимости. */
  const hk = clampN(S.height / 2.2, 0.88, 1.5);
  const LIFE = dur + 0.5;
  ...
  const rs = restriker(field, seed, (t, g) => {
    ...
    if (t < S.takeoff) {
      /* Шесть нитей от низа капсулы наружу по полу: 1.2–2.2 м на стендовой
         высоте 2.2 и не короче 1.06 м на самом низком прыжке (см. зажим
         `hk` выше). Замер i1 — четыре нити по 0.5–1.1 м с 26 м были
         невидимы, кадр 0.12 с пустой. */
      for (let i = 0; i < S.threads; i++) {
