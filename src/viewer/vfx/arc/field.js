...
function bundleSegs(s, rng, put) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const straightP = 0.3 + rng() * 0.35;
    const throwK = 0.7 + rng() * 0.8;
    ...
    for (let k = ks + 1; k <= ke; k++) {
      ...
      /* Поворотное звено: размах 0.35–1.5 шага (свой у нити через `throwK`)
         и не больше 2.2 радиуса трубы здесь — у руки труба 3 см, нити
         выходят почти прямым веером, как у эталона. */
      const jump = Math.min(Math.max(vl, stepL * 0.35), stepL * (0.5 + rng() * 0.55) * wander * throwK, rad * 2.2 + 0.03);
