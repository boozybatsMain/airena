...
export function cone(vfx, e, P, ctx) {
   ...
   * Теперь капли ложатся на 3.1–3.8 м, то есть по самой кромке. Скорость
   ...
   */
  const reach = Math.max(0.5, range * S.reach - S.hand);
  const landed = [];
  for (let i = 0; i < n; i++) {
...
export function bolt(vfx, e, P, ctx) {
  ...
  const S = kit.tune(e, {
    ...
    pour: 0.68,       /* окно слива, доли времени полёта */
    pourMax: 0.6,     /* потолок окна слива, с */
  ...
  const pour = Math.min(S.pourMax, (len / speed) * S.pour);
  const landed = [];
...
export function zone(vfx, e, P, ctx) {
  ...
  const S = kit.tune(e, {
    ...
    decalRise: 1.6,   /* за сколько РАЗЛИВАЕТСЯ лужа, с */
  ...
  fumes(vfx, P, { x: e.x, z: e.z, r: r * 0.9, n: clampN(Math.round(r * 10), 12, 40), life: D * S.fume, rng });
  return true;
}
