...
/*
 * ГРАДУИРОВКА ПОД МИР (ARENA-AAA, 06.09). Потолок картинки — копинг
 * #F4EEE8, L* 94 (ARENA-BRIEF §3: «nothing exceeds»), а бело-горячее шло
 * L* 98.4, то есть светлее всего, что в этом мире вообще бывает, — и это ДО
 * своего блума. Оно осталось самым светлым в огне и перестало быть светлее
 * мира: #EFEBDF, L* 93.1, тот же тёплый оттенок (b* +6).
 */
const WHITE_HOT = new THREE.Color(0.86, 0.83, 0.74);
/*
 ...
 */
/* Цветность прижата с C* 37 до 32 (насыщенность 42 % → 36 %) — ровно до
   полки, на которой стоят градуированные ступени палитры (`vfx.js`,
   `gradeStop`). Довод выше не тронут: это тот же янтарь и та же разница по
   тону, просто он больше не спорит с коралловым баннером за место самого
   насыщенного предмета кадра. */
const BOLT_CORE = new THREE.Color(0.95, 0.67, 0.35);
 ...
 */
/*
 * КРУГ 8 (ARENA-AAA, 06.09): РЕДЕЮЩИЙ ДЫМ УХОДИТ В СТОЛ, НО НЕ СТАНОВИТСЯ
 * СТОЛОМ. Довод выше цел — на белом полу редеющий обязан светлеть, а не
 * чернеть, — но 0.44/0.42/0.40 это L* 71 при холодном b* +2, то есть ровно
 * тот «ещё немного площади», за который приёмка отвергла шлейф на
 * `live-visitor-m.png`: «столб пыли в том же оттенке и той же полосе
 * светлоты, что и площадь… глубины не добавляет». Стало L* 67.6 при b* +7 —
 * двадцать уровней под полом (L* 88) и в тёплой полосе мира: дым, а не
 * вторая площадь.
 */
const SMOKE_THIN = new THREE.Color(0.40, 0.37, 0.33);
const SOOT = new THREE.Color(0.05, 0.045, 0.04);   /* L* 25.5 — светлее бойца */
const BASALT = new THREE.Color(0.13, 0.11, 0.10);
const ASH = new THREE.Color(0.44, 0.42, 0.40);

/*
 * ── БЮДЖЕТ ШЛЕЙФА (ARENA-AAA, приёмка 06.09) ──────────────────────────────
 *
 * ЖАЛОБА, ДОСЛОВНО. `live-visitor-m.png` (390×844): «столб пыли заполняет
 * (95-215, 240-355) в том же оттенке и той же полосе светлоты, что и
 * площадь (L* 75-85), и ПРЯЧЕТ ОБОИХ СУЩЕСТВ ОТ КОЛЕНА ВНИЗ — два тела при
 * (95-215, 215-300) висят без ног, а их кольца стоят на 60-100 px ниже.
 * Ни один боец не касается земли, на которой он помечен, и, поскольку столб
 * одной светлоты с полом, взамен он не даёт никакой глубины».
 *
 * Замер повторён: середина коробки судьи даёт L* 77.8 при b* +13 против
 * чистой плиты L* 84.8 при b* +6.9. То есть столб СЕМЬ уровней под полом —
 * на глаз это одна поверхность, а площадь он занимает как предмет.
 *
 * ЧТО ЗДЕСЬ РЕЖЕТСЯ И ЧТО НЕТ. Огонь — телеграф: круг на полу, языки, время
 * жизни и число частиц читает игрок, и ни одно из них не тронуто. Тронута
 * ВЫСОТА и ШИРИНА КЛУБА — то, чем шлейф залезает на тела и на их метки:
 *   · `rise` (и `lift`, тот же вектор) ×0.72 у языков и у дыма: столб зоны
 *     шёл в среднем на 4.8 м при бойце в 2.2 — то есть вдвое выше того, что
 *     он закрывал; теперь 3.5 м, и линия «тело → кольцо» открыта;
 *   · `size` дыма ×0.82 и `grow` ×0.85: площадь одного клуба к смерти
 *     падает вдвое, а перекрытие — то, чем вуаль набирает плотность, — с
 *     ним. Языки по ширине не тронуты: ширина языка и есть силуэт огня.
 * Одна ручка на модуль вместо двенадцати правок по местам вызова: место
 * вызова описывает СВОЙ очаг, бюджет — общий для стихии.
 */
const PLUME = { rise: 0.72, size: 0.82, grow: 0.85 };
...
function emitFlames(vfx, n, place, o) {
  const {
    P, y = 0.2, yJit = 0.4, rise: rise0 = 2.4, life = 0.7, size = 0.9, at = null, jitter = 0.2,
    vx = 0, vz = 0, out = 0.5, lift: lift0 = 1.2, hotK = 0.5, stretch = 0, r = Math.random, spin = 0.6,
    soft = 3, /* каждый `soft`-й язык — формой дыма (0 или 1 — выключено) */
  } = o;
  /* Столб короче на четверть — тем же бюджетом, что и дым (см. `PLUME`).
     Ширина языка не тронута: она и есть силуэт огня. */
  const rise = rise0 * PLUME.rise;
  const lift = lift0 * PLUME.rise;
...
function emitSmoke(vfx, n, place, o) {
  const {
    dark = SMOKE, lit = null, thin = SMOKE_THIN, y = 0.5, yJit = 0.5, rise: rise0 = 2.2, life = 2.0,
    size: size0 = 1.1, at = null, jitter = 0.25, out = 1.0, grow = 1.85 * PLUME.grow, r = Math.random,
  } = o;
  /* БЮДЖЕТ ШЛЕЙФА — ОДНОЙ РУЧКОЙ (см. `PLUME`): столб ниже и уже, а все
     двенадцать мест, откуда его зовут, остались нетронутыми. */
  const rise = rise0 * PLUME.rise;
  const size = size0 * PLUME.size;
  const born = at ?? vfx.now;
  vfx.body.emit(n, (i, s) => {
    const [px, pz] = place(i);
    const a = r() * Math.PI * 2, k = rnd(0.2, 1, r) * out;
